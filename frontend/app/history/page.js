  ...
  return (
    <div className="w-full max-w-4xl mx-auto px-6 py-12 flex flex-col gap-8">
      ...
      <div className="flex flex-col gap-1.5 text-left">
        <h1 className="text-2xl font-extrabold tracking-tight text-text-charcoal dark:text-text-offwhite font-heading flex items-center gap-2">
          <Clock className="w-6 h-6 text-primary-green-light dark:text-primary-green-dark" />
          Search History
        </h1>
      ...
      {loading ? (
        ...
        <div className="flex flex-col gap-4 max-w-3xl mx-auto w-full">
          {[...Array(4)].map((_, i) => (
            <div 
              key={i} 
              className="w-full h-24 rounded-2xl bg-slate-500/5 dark:bg-slate-500/5 border border-sage-gray-light/10 dark:border-sage-gray-dark/10 animate-pulse flex flex-col gap-3 p-5"
            >
        ...
        <div className="glass-card p-8 rounded-3xl flex flex-col items-center justify-center text-center gap-4 max-w-md mx-auto">
          ...
          <h3 className="text-sm font-bold text-text-charcoal dark:text-text-offwhite font-heading">Failed to Load History</h3>
          <p className="text-xs text-sage-gray-light dark:text-sage-gray-dark">{error}</p>
          <button 
